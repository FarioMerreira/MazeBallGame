...
Player player = {{1.0f, 1.0f}, 3}; // Jogador começa na posição (1.0, 1.0) com 3 luzes
...
std::pair<float, float> end_point = {12.0f, 7.0f}; // Ponto de saída do labirinto

//Configurações da câmera isométrica
float camera_distance = 5.0f; // Distância da câmera
float spin = 0.0f; // Rotação da câmera
float light_direction[2] = {0.0f, 1.0f}; // Direção inicial da luz
float camera_angle = 45.0f; // Ângulo da câmera
float camera_height = 10.0f; // Altura da câmera

//Função para inicializar o labirinto
void initMaze() {
    // Definindo as paredes do labirinto como pares de coordenadas (x, y)
    maze_walls = {
        // Bordas externas
        {0.0f, 0.0f}, {15.0f, 0.0f}, // Parede inferior
        {0.0f, 15.0f}, {15.0f, 15.0f}, // Parede superior
        {0.0f, 0.0f}, {0.0f, 15.0f}, // Parede esquerda
        {15.0f, 0.0f}, {15.0f, 15.0f}, // Parede direita

        // Paredes internas baseadas no desenho:

        // Região superior
        {1.0f, 0.0f}, {1.0f, 3.0f}, // Parede vertical
        {3.0f, 0.0f}, {3.0f, 1.0f}, // Parede vertical curta
        {4.0f, 1.0f}, {7.0f, 1.0f}, // Parede horizontal
        {6.0f, 0.0f}, {6.0f, 1.0f}, // Parede vertical curta
        {8.0f, 0.0f}, {8.0f, 4.0f}, // Parede vertical
        {10.0f, 2.0f}, {14.0f, 2.0f}, // Parede horizontal longa
        {12.0f, 0.0f}, {12.0f, 2.0f}, // Parede vertical curta

        // Região central
        {3.0f, 5.0f}, {2.0f, 7.0f}, // Parede diagonal
        {4.0f, 3.0f}, {6.0f, 3.0f}, // Parede horizontal
        {5.0f, 3.0f}, {5.0f, 5.0f}, // Parede vertical
        {7.0f, 4.0f}, {9.0f, 4.0f}, // Parede horizontal
        {9.0f, 4.0f}, {9.0f, 8.0f}, // Parede vertical
        {11.0f, 5.0f}, {13.0f, 5.0f}, // Parede horizontal
        {11.0f, 5.0f}, {11.0f, 8.0f}, // Parede vertical
        {13.0f, 5.0f}, {13.0f, 7.0f}, // Parede vertical curta

        // Região inferior
        {1.0f, 8.0f}, {5.0f, 8.0f}, // Parede horizontal
        {3.0f, 8.0f}, {3.0f, 12.0f}, // Parede vertical
        {5.0f, 9.0f}, {7.0f, 9.0f}, // Parede horizontal
        {7.0f, 9.0f}, {7.0f, 13.0f}, // Parede vertical
        {9.0f, 10.0f}, {11.0f, 10.0f}, // Parede horizontal
        {9.0f, 10.0f}, {9.0f, 14.0f}, // Parede vertical
        {11.0f, 11.0f}, {13.0f, 11.0f}, // Parede horizontal
        {13.0f, 11.0f}, {13.0f, 13.0f}, // Parede vertical

        // Pequenas paredes no canto inferior
        {1.0f, 14.0f}, {6.0f, 14.0f}, // Parede horizontal
        {8.0f, 14.0f}, {14.0f, 14.0f}, // Parede horizontal longa
    };
...
void drawMaze() {
    for (auto &wall : maze_walls) { // Loop pelas paredes do labirinto
        ...
        //Checando se a parede é uma borda
        ...
        glTranslatef(wall.first, wall.second + (is_boundary_wall ? 2.5f : 2.0f), 0.0f); // Centraliza a parede
        if (is_boundary_wall) {
            glScalef(1.5f, 5.0f, 3.0f); // Maior escala para paredes de borda
        } else {
            glScalef(1.0f, 4.0f, 3.0f); // Escala padrão para paredes internas
        }
...
void gameDisplay() {
    ...
    if (game_over) {
        ...
        glMatrixMode(GL_MODELVIEW); // Define a matriz de modelo/visualização
        glLoadIdentity(); // Carrega a matriz identidade
        drawTextGame(glutGet(GLUT_WINDOW_WIDTH)/2 - 100, glutGet(GLUT_WINDOW_HEIGHT)/2, "YOU WIN! Press spacebar to start NG+ (harder difficulty)"); // Desenha o texto
        ...
        if (light.thrown && !light.hit_wall) { // Se a luz foi lançada e não atingiu parede
            ...
            for (auto &wall : maze_walls) { // Loop pelas paredes
                if (fabs(light.pos[0] - wall.first) < 0.3f && 
                    ...
                    light.pos[1] <= wall.second + 4.2f) { // Verifica colisão
    ...
    switch (key) {
        ...
        case ' ':
            if (game_over) { // Se o jogo terminou
                ...
                game_over = false;
                zoom_out = false;
                player = {{2.0f, 2.0f}, 3}; // Reseta o jogador
...
void switchToGameMode() {
    ...
    //Resetando jogo
    player = {{2.0f, 2.0f}, 3}; // Posição inicial do jogador
